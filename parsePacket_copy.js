  ...
  return new Promise(async function (resolve, reject) {
    ...
    if (request.packetType == packetTypes.login && packetLength == 13) {
      ...
      if (errorCheckValid) {
        ...
        resolve(null);
        console.log(request.imei + " requestimei11  :");
      ...
      resolve(response);
      // }else{
      //   console.log( FuncTag+ "Boom")
      // }

      console.log(request.imei + " requestimei121  :");
      ...
      var date = parseDate(dataArray, 4, request.gmt);
      ...
      console.log(request.imei + " requestimei33 :");
      console.log(request.mmcCountryCode + " mmcCountryCode :");
      ...
      if (locationSimilar) {
        ...
        if (!request.imei) {
          ...
          request.imei = '865468050777740';
        }


        // Increment the RowID by 1
        let rowid = 1;

        console.log(
          + " request: ");
        // var requestJSON = JSON.stringify(request);
        // console.log(FuncTag + config1)
        const config1 = {
          "RowID": "" + rowid,
          "DeviceID": request.imei,
          "ConfigVersionId": request.timestamp,
          "MaxValue": request.voltage,
          "VehicleDirection": request.gpsInfoLength,
          "DateAndTimeStart": request.date = "" + request.time,
          "DateAndTimeEnd": request.date = "" + request.time,
          "EventID": request.charge,
          "AccumulatedDistance": "true",
          "Lat": request.latitude,
          "Lng": request.longitude,
          "VehicleStatus": request.ACC,
        ...
        // "VehicleSpeed": request.speed,
        // "odometerDistance": odometerDistance
        console.log("string" + request.date + ' ' + request.time)
        console.log("ACC" + request.ACC)
        ...
        if (!request.imei) {
          throw new Error('request.imei is not defined');
          // or assign a default value
          // request.imei = 'default_imei';
        }
        //  //console.log("imei!!!"+imei)
        // console.log("imei!!!"+config1)
        const config5 = {
          "RowID": "" + rowid,
        ...
        const config2 = JSON.stringify(config1)
        const config4 = JSON.stringify(config3)
        const config6 = JSON.stringify(config5)



        sendDataTestOtofusionURL(config2, "gps");
        sendAxios(config2, "gps");
        sendDataTestOtofusionURL(config4, "event");
        sendAxios(config4, "event");
        sendDataTestOtofusionURL(config5, "event");
        sendAxios(config5, "event");
        ...
        //   + " request: ");
        var requestJSON = JSON.stringify(request);
        console.log(FuncTag + requestJSON)

        // sendDataTestOtofusionURL(requestJSON, " requestJSON ");
        // sendAxios(requestJSON, " requestJSON ");
      }
...
function parsePositionAzimuth(dataArray, request, startIndex, endIndex) {
  ...
  var firstByteBits = positionBytesArray[0].toString(2);
  while (firstByteBits.length < 8) {
    firstByteBits = "0" + firstByteBits;
  }
  var secondByteBits = positionBytesArray[1].toString(2);
  while (secondByteBits.length < 8) {
    secondByteBits = "0" + secondByteBits;
  }
  ...
  if (firstByteBits[2] == "0") {
    request.gpsRealTime = 1;
  } else {
    request.gpsRealTime = 0;
  }
  if (firstByteBits[3] == "1") {
    request.gpsPositioned = 1;
  } else {
    request.gpsPositioned = 0;
  }
  if (firstByteBits[4] == "1") {
    request.longitudeDirection = "west";
  } else {
    request.longitudeDirection = "east";
  }
  if (firstByteBits[5] == "1") {
    request.latitudeDirection = "south";
  } else {
    request.latitudeDirection = "north";
  }

  secondByteBits =
    firstByteBits[6] + "" + firstByteBits[7] + "" + secondByteBits;
  request.azimuth = parseInt(parseInt(secondByteBits, 2));
  return request;
}
...
async function checkIgnitionFromHeartbeat(request, user) {
  ...
  if (user) {
    if (user.ignition.status) {
      ...
      console.log(
        FuncTag + "user doesn't want to receive ignition notification"
      );
    }
  } else {
    console.log(FuncTag + "user doesn't exist");
