...
async function fetchAndDisplayJSON() {
    try {
        ...
        displayTopics(Object.keys(originalData));
    } catch (error) {
...
function displayTopics(topics) {
    topics.forEach((topic) => {
        ...
        topicItem.addEventListener('click', () => {
            const topicData = originalData[topic];
            displayJSON(topicData);
        });
    });
}
...
editBtn.addEventListener('click', () => {
    ...
    try {
        ...
        // Clear existing entries in key-value list
        keyValueList.innerHTML = '';

        // Select the first topic by default
        const defaultTopic = Object.keys(originalData)[0];
        const defaultTopicData = originalData[defaultTopic];

        createInputsForObject(defaultTopicData);
